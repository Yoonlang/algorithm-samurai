...
var NestedIterator = function (nestedList) {
  ...
  const handleNestedInteger = (ni) => {
    if (ni.isInteger()) {
      this.arr.push(ni.getInteger());
    } else {
      ni.getList().forEach((n) => {
        handleNestedInteger(n);
      });
    }
  };

  nestedList.forEach((l) => {
    handleNestedInteger(l);
  });
  this.i = 0;
};
...
NestedIterator.prototype.hasNext = function () {
  return typeof this.arr[this.i] === "number" ? true : false;
};
